...
const getAllEquipamentos = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const equipamentos = await db('Equipamentos').limit(Number(limit)).offset(offset)
...
const getEquipamentoById = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const equipamento = await db('Equipamentos').where({ id: req.params.id }).first();
    if (!equipamento) {
...
const createEquipamento = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const [newEquipamento] = await db('Equipamentos').insert({ nome, categoria, atributoPrim, valorAtri }).returning('*');
...
const updateEquipamento = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const updatedEquipamento = await db('Equipamentos').where({ id: req.params.id }).update({ nome, categoria, atributoPrim, valorAtri }).returning('*');
    res.status(200).json({
...
const patchEquipamento = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const updatedEquipamento = await db('Equipamentos').where({ id: req.params.id }).update(updates).returning('*');
    res.status(200).json({
      message: 'Equipamento atualizado!',
      equipamento: updatedEquipamento
    });
...
const deleteEquipamento = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    await db('Equipamentos').where({ id: req.params.id }).del();
